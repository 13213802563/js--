...
requirejs(["jquery", "config", "util", 'widgets', 'bootstrap.table', 'bootstrap.local', 'bootstrap.validate'], function ($, conf) {
  ...
  // 页面初始化
  yt.init = function () {
    yt.tableInit({ store_location: "STO0049" });
    ...
    util.requestData({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < res.length; i++) {
          if (res[i].up_store_id === "STO0049") {
            newRes.push(res[i]);
          }
        }
        dataArr.push({ v: "STO0049", t: "全部"})
  ...
  $(".derive").on("click", function () {
    ...
    util.requestData({
      ...
      success: function (res) {
        var res = res.reverse()
        if (titleText === "全部") titleText = "镇街仓库全部"
  ...
  $(".print").on("click", function () {
    ...
    util.requestData({
      ...
      success: function (res) {
        ...
        if (titleText === "全部") titleText = "镇街仓库全部"
        var title = '<p style="text-align: center;  font-size: 28px; ">' + titleText + '防汛物资统计</p>'
        if (res.length > count) {
          firstSum = count
        } else {
          firstSum = res.length;
  ...
  function render(title, res, count, sum) {
    var html = title +
      '<table border="1" cellspacing="0"  cellpadding="5" class="realHtml" style="border-collapse:collapse;font-size: 9px; text-align: center; width:100%">' +
      '<tr>' +
      '<td>序号</td>' +
      '<td>物质名称</td>' +
      '<td>规格</td>' +
