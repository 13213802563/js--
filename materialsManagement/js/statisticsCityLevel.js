...
requirejs(["jquery", "config", "util", 'widgets', 'bootstrap.table', 'bootstrap.local', 'bootstrap.validate'], function ($, conf) {
  ...
  // 页面初始化
  yt.init = function () {
    yt.tableInit({ store_location: "STO0048" });
    ...
    util.requestData({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < res.length; i++) {
          if (res[i].up_store_id === "STO0048") {
            newRes.push(res[i]);
          }
        }

        dataArr.push({ v: "STO0048", t: "全部"})
  ...
  $(".derive").on("click", function () {
    ...
    if (titleText === "全部") titleText = "市级仓库全部"
  ...
  function render(title, res, count, sum) {
    ...
    for (var i = count; i < sum; i++) {
      console.log(res[i].material_name)
      html += '<tr class="tbody">' +
        '<td>' + (i + 1) + '</td>' +
