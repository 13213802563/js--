...
+function ($) {
  ...
  PTR.prototype.touchMove= function(e) {
    ...
    if (Math.abs(this.diffX) > Math.abs(this.diffY)) return true; // 说明是左右方向的拖动if($(document).scrollTop()!=0) return false;
  ...
  var getOffset = function (container) {
    ...
    if (tagName === 'BODY' || tagName === 'HTML') {
      scrollTop = container.scrollTop() || $(window).scrollTop();
        offset=container.scrollHeight() - ($(window).height() + scrollTop);
    } else {
      scrollTop = container.scrollTop();
        offset=container.scrollHeight() - (container.height() + scrollTop);
    }
    return offset
  }
